...
struct dpll_param {
	...
	int dpll_id;
	int dpll_source_id;
	enum dpll_genl_signal_type dpll_source_type;
	int dpll_output_id;
	enum dpll_genl_signal_type dpll_output_type;
	int dpll_status;
					   ...
					   struct sk_buff *msg)
{
	...
	for (i = 0; i < dpll->sources_count; i++) {
		...
		type = ops->get_source_type(dpll, i);
		...
		if (ops->get_source_supported) {
			for (type = 0; type <= DPLL_TYPE_MAX; type++) {
				ret = ops->get_source_supported(dpll, i, type);
					   ...
					   struct sk_buff *msg)
{
	...
	for (i = 0; i < dpll->outputs_count; i++) {
		...
		type = ops->get_output_type(dpll, i);
		...
		if (ops->get_output_supported) {
			for (type = 0; type <= DPLL_TYPE_MAX; type++) {
				ret = ops->get_output_supported(dpll, i, type);
...
static int dpll_genl_cmd_set_source(struct sk_buff *skb, struct genl_info *info)
{
	...
	enum dpll_genl_signal_type type;
	int ret = 0, src_id;
...
static int dpll_device_loop_cb(struct dpll_device *dpll, void *data)
{
	...
	struct dpll_param *p = (struct dpll_param *)data;
...
dpll_cmd_device_dump(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	struct dpll_param p = { .cb = cb, .msg = skb };

	return for_each_dpll_device(ctx->pos_idx, dpll_device_loop_cb, &p);
}

static int
dpll_genl_cmd_device_get(struct sk_buff *skb, struct genl_info *info)
{
	struct dpll_device *dpll = info->user_ptr[0];
	struct nlattr **attrs = info->attrs;
	struct sk_buff *msg;
				   ...
				   struct param *p)
{
	...
	genlmsg_multicast(&dpll_gnl_family, msg, 0, 0, GFP_KERNEL);
