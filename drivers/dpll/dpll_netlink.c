...
static int dpll_genl_cmd_set_source_prio(struct param *p)
{
	const struct nlattr **attrs = (const struct nlattr **)p->attrs;
	struct dpll_device *dpll = p->dpll;
...
static int dpll_genl_cmd_set_select_mode(struct param *p)
{
	const struct nlattr **attrs = (const struct nlattr **)p->attrs;
	struct dpll_device *dpll = p->dpll;
	int ret = 0, mode;
...
static const cb_t cmd_doit_cb[] = {
	...
	[DPLL_CMD_SET_SRC_SELECT_MODE] = dpll_genl_cmd_set_select_mode,
	[DPLL_CMD_SET_SOURCE_PRIO]     = dpll_genl_cmd_set_source_prio,
};
