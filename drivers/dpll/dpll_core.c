...
static int pin_register(struct xarray *xa_pins, struct dpll_pin *pin)
{
	struct dpll_pin *pos;
	unsigned long index;
	int ret;
	...
	ret = xa_alloc(xa_pins, &id, pin, xa_limit_16b, GFP_KERNEL);
	if (!ret)
		xa_set_mark(xa_pins, id, PIN_TYPE_TO_MARK(pin->type));
...
EXPORT_EXPORT_GPL(dpll_muxed_pin_free);
